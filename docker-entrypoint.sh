...
# BASE_URL is the externally visible URL for Headwind MDM. It allows TLS to be
# terminated by a reverse proxy while Tomcat continues to listen over plain HTTP.
# Preserve the original behavior when BASE_URL is not set.
if [ -z "$BASE_URL" ]; then
...
if [ ! -f "$TOMCAT_DIR/conf/Catalina/localhost/ROOT.xml" ] || [ "$FORCE_RECONFIGURE" = "true" ]; then
    cat $TEMPLATE_DIR/conf/context_template.xml | sed "s|_SQL_HOST_|$SQL_HOST|g; s|_SQL_PORT_|$SQL_PORT|g; s|_SQL_BASE_|$SQL_BASE|g; s|_SQL_USER_|$SQL_USER|g; s|_SQL_PASS_|$SQL_PASS|g; s|_BASE_URL_|$BASE_URL|g; s|_SHARED_SECRET_|$SHARED_SECRET|g;" > $TOMCAT_DIR/conf/Catalina/localhost/ROOT.xml
fi
...
cat $BASE_DIR/init1.sql | sed "s|https://h-mdm.com|$BASE_URL|g" > $BASE_DIR/init.sql
...
# jks is always created from the certificates when Tomcat itself terminates HTTPS
